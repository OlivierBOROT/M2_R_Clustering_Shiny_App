    ...
    validate_data = function() {
      ...
      for (col_name in names(self$data)) {
        ...
        if (!is.numeric(self$data[[col_name]]) && !is.factor(self$data[[col_name]])) {
          stop(sprintf(
            "Variable '%s' has unsupported type '%s'. Only numeric and factor are allowed.",
  ...
  public = list(
    ...
    print = function() {
      ...
      if (private$actual_runs < private$n_init) {
        cat(sprintf(
          "  Convergence after %d iterations (init: %s, runs=%d/%d - early stop)\n",
          private$n_iterations, private$init_method,
          private$actual_runs, private$n_init
        ))
      } else {
        cat(sprintf(
          "  Convergence after %d iterations (init: %s, runs=%d)\n",
    ...
    summary = function() {
      ...
      if (private$actual_runs < private$n_init) {
        cat(sprintf(
          "  - Initialization runs: %d / %d (early stopped)\n",
          private$actual_runs, private$n_init
        ))
      } else {
      ...
      for (j in 1:self$n_clusters) {
        ...
        cat(sprintf(
          "    First dimension explains %.1f%% of cluster variance\n",
          homog * 100
        ))
      }
