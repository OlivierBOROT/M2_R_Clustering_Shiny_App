  ...
  public = list(
    ...
    fit = function() {
      ...
      if (is_pure_numeric) {
        ...
        if (!requireNamespace("PCAmixdata", quietly = TRUE)) {
          stop("Package 'PCAmixdata' is required for mixed (numeric + categorical) data.\n",
      ...
      while (current_k < self$n_clusters) {
        ...
        if (self$stop_at_kaiser && !is.na(max_eigenvalue_2) && max_eigenvalue_2 < 1.0) {
          cat(sprintf("Kaiser criterion: Stopping at %d clusters (max lambda2 = %.3f < 1.0)\n", 
        ...
        if (!is.na(max_eigenvalue_1) && !is.na(max_eigenvalue_2) && max_eigenvalue_1 > 0) {
          ...
          if (ratio < self$min_eigenvalue_ratio) {
            cat(sprintf("Eigenvalue ratio criterion: Stopping at %d clusters (lambda2/lambda1 = %.3f < %.3f)\n",
        ...
        self$split_history[[iteration]] <- list(
          ...
          eigenvalue_ratio = if (!is.na(split_result$eigenvalue_1) && split_result$eigenvalue_1 > 0) 
                               split_result$eigenvalue_2 / split_result$eigenvalue_1 else NA,
      ...
      cat(sprintf("Divisive clustering completed: %d clusters formed in %d iterations\n", 
    ...
    predict = function(new_data) {
      ...
      if (nrow(new_data) != nrow(self$data)) {
        stop(sprintf("New data must have the same number of observations as training data (%d vs %d)",
    ...
    print = function() {
      ...
      if (self$fitted) {
        ...
        for (i in seq_along(self$split_history)) {
          ...
          cat(sprintf("  Iteration %d: Split cluster %d (lambda1=%.3f, lambda2=%.3f%s) -> %d clusters\n",
      ...
      for (k in seq_len(self$n_clusters)) {
        ...
        if (length(self$cluster_eigenvalues[[k]]) > 0) {
          ...
          if (length(self$cluster_eigenvalues[[k]]) > 1) {
            ...
            cat(sprintf("  Eigenvalue ratio (lambda2/lambda1): %.3f\n", 
